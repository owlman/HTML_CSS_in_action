...
企业网站的首页设计在网页设计领域中属于企业展示类项目，其设计目的是让目标网页成为用户进入企业网站时的门户，以便向用户展示该企业的品牌信息，并为他们后续访问动作提供进一步的导航功能。在此类项目中，网页设计师们一方面会充分利用色彩、图片等最为直接的视觉元素来迅速建立企业的品牌形象，另一方面也会通过合理的页面布局设计来有效地展示当前网站所能提供给用户的主要信息，并引导他们快速找到自己所希望了解的内容。因此，企业网站的首页设计也被认为是网页设计师们在进入到网页设计领域时首先要学会做的基础项目之一。

## 【学习目标】

在本章，笔者将会以一家名为“凌雪冰熊”的连锁饮料店的需求为例来演示如何完成企业网站的首页设计。该演示项目的设计目标是向公众展示该饮料店的品牌信息，以便建立起人们对这家连锁品牌的第一印象。同时，该项目也会为该企业网站后续要设计的新闻活动、产品展示、申请加盟等页面设置好统一的布局风格与配色方案，并预留整合这些页面的导航链接。通过本章项目的实践，读者将会初步了解设计一个企业展示类页面所要执行的基本步骤，以及执行这些步骤所需的基本技术与相关工具。总而言之，在阅读完本章之后，我们希望读者能够：

- 了解HTML 5中提供的布局类标记，并掌握这些标记在网页设计工作中的具体使用；
- 了解在开展网页设计工作时首先要解决的尺寸设置、配色选择等整体设计问题；
- 掌握如何在项目中引入Bootstrap框架，并用该框架来完成网页的整体设计任务；

## 【学习场景描述】

现在你是一位刚刚入职到一家名为“凌雪冰熊”连锁饮料店的网页设计师。该连锁店的领导层决定为企业开发一个官方网站，以便向公众更好地展示自己的品牌形象，企业最新的活动，并为潜在的合作伙伴提供咨询信息，进一步扩展线下实体店的加盟规模。在这个网页设计项目中，你的主要任务有两个：首先是为该企业网站完成首页部分的设计，以便为该连锁饮料店建立良好的品牌印象。其次，你需要为该网站设计统一的布局风格与外观样式，为后续要进行的新闻活动、产品展示、申请加盟等网页的设计项目打好基础。
...
- **委托方**：凌雪冰熊股份有限公司互联网部门
- **项目资料**：
  - **图片资料**：主要包括品牌的Logo图一张，代表性门店的照片三张（如图2-1所示）。  
    ![pic](./img/2-1.png)  
    **图1-1**：凌雪冰熊网站的图片资料
- **项目要求**：为凌雪冰熊连锁饮料店的官方网站设计首页，该网页的设计应符合以下要求。
  - 该网页应要呈现与凌雪冰熊品牌Logo一致的配色风格；
  - 该网页应立足于整个网站来设计统一的整体布局风格；
...
- 创建凌雪冰熊官方网站的网页设计项目，并在项目中引入Bootstrap框架；
- 创建凌雪冰熊官方网站的首页，并使用HTML标记完成该页面的整体布局；
- 基于Bootstrap框架提供的样式类和组件来完成网站首页的样式设计任务；

## 【工作准备】

在经过了上一章的项目实践之后，读者想必已经对在网页设计工作中会用到的两门计算机标记语言，HTML和CSS有了一个基本的了解。接下来，本书将根据要实践的项目场景来分门别类地介绍这两门语言及其相关框架的具体应用。在本章要实践的项目中，我们的主要任务是完成企业类网站首页的整体设计，目的是为整个网站建立统一的布局风格和配色方案。下面就先来介绍一下完成该项目任务所需要掌握的知识点与工具。当然了，如果读者认为自己已经掌握了这部份知识，可自行跳过本节内容，直接进入本章项目的【工作实施与交付】环节。

### 知识点1：HTML 5中的布局类标记及其用法

和画家在拿到画布之后要先进行整体的构图作业一样，网页设计师们在定义好一个网页文档之后，接下来要完成的工作是网页的布局设计。在上一章的项目中，读者用`<div>`标记和CSS中基于`id`属性的选择器在网页中绘制出了一个类似于名片形状的圆角矩形，这个动作就可以被视为网页的布局。在这个动作完成之后，设计师们就可以在这个圆角矩形中填充与名片相关的信息了。在HTML 5标准发布之前，网页的布局工作也基本上是依靠`<div>`标记搭配相关的CSS属性选择器来完成的，这种方式在一定程度上给项目代码的可读性带来不良的影响，这些影响又进而会给项目的维护工作带来不少麻烦。对此，读者只需想象一下，当同一个HTML文档中遍布着数十个甚至上百个时而并列、时而嵌套的`<div>`标签时会是什么情况，就不难理解自己会遇到怎么样的麻烦了。为了更好地解决这一类因代码可读性所带来的问题，HTML5标准中新增了许多专用于网页布局的标记，下面是这些标记的基本使用示范。
...
从本质上来说，HTML 5中新增的这些布局类标记都可被视为`<div>`标记的别名，它们只不过是语义化了该标记的一些特定应用场景。这样做不仅有利于提高HTML代码的可读性，以便降低网页设计项目的维护难度，还能提升网页对搜索引擎的友好度，使得相关信息更容易被找到。
...
### 知识点3：Bootstrap框架的基本使用

到目前为止，本书所演示的都是基于手工编码的方式来进行网页设计的。这种从零开始编写HTML+CSS代码的做法对于网页设计教学来说是必要的，它能让初学者以“在做中学，学中做”的方式来实现快速入门，但在实际生产环境中，它就不见得是最佳实践了，因为这种做法不仅非常耗时费力，而且极易出错，对网页设计师的要求也相对较高。如果读者平常只是一个前端程序员，并没有经历过专业的美术训练，采用这种手工编码方式就大概率会在用户界面设计、配色方案设计上遇到较大的挑战。因此，在实际项目实践中，设计师们往往更倾向于使用成熟的第三方框架来辅助进行网页设计的工作。从本章开始，本书将致力于介绍如何基于Bootstrap框架来完成网页的设计工作。
...
总而言之，Bootstrap框架是一款功能强大、易用性高、可扩展性强的网页设计框架，它为设计师们提供了快速构建响应式布局和常见Web界面元素的工具，极大地提高了开发效率和用户体验。如果读者想成为一名前端开发者，Bootstrap框架绝对是你应该要学习的工具之一。截止到本书撰写的时间为止（即2023年12月），Bootstrap框架已经迭代到了5.x版本系列，它相对于4.x和3.x最大的区别在于JavaScript脚本部分的实现，如今的Bootstrap框架在操作DOM时会直接使用ECMAScript 6的原生接口，不再需要额外引入jQuery库了。

本书将基于5.x这一系列的版本来介绍如何使用Bootstrap框架来构建网页，下面，笔者将根据本章项目的任务需要来介绍该框架的使用方法。正如之前所说，Bootstrap框架之所以如此受到欢迎，主要因为它提供了大量可重用的界面组件和CSS样式，这些组件和样式可以帮助设计师们快速有效地完成网页的整体设计任务。具体来说，Bootstrap框架在网页整体设计方面可以提供的便利主要如下：
...
下面，让我们先从网页的布局设计开始。在网页的整体布局方面，Bootstrap框架为网页设计师们提供了以下几种常见的布局方式及其相关的样式类：
...
当然了，除了选择上面其中一种布局方式之外，读者还可以根据自己的具体需求灵活地混合使用这些方式，以便创建出更具复杂性的网页。Bootstrap框架的灵活性及其提供的丰富文档资源可以帮助我们轻松实现各种复杂的网页布局设计。下面，让我们来演示一下如何基于Bootstrap框架来实现与图2-2所示网页相似的布局效果，演示步骤具体如下。

1. 在本地计算机中创建一个名为`BootstrapCase`的目录（在本书的源码包中，笔者将会将其创建在`examples/00_demo`目录中），并在其中创建一个名为`index.htm`的网页文件和两个分别名为`styles`和`scripts`的子目录。

2. 打开网页浏览器，使用搜索引擎找到Bootstrap框架的官网，然后进入到如下图所示的下载页面，并单击图中的「Download」按钮将编译好的CSS和JavaScript文件下载到本地计算机中。当然了，如果能用NPM包管理器来构建项目的话，笔者在这里更推荐读者先使用Powershell或Bash Shell这类命令行终端环境进入到之前创建的`bootstrapCase`目录下，然后再通过执行`npm install bootstrap@5.3.2 --dev`命令来下载该框架。

    ![图2-4](./img/2-4.png)

    **图2-4**：Bootstrap官方下载页面

3. 下载完成后，读者会得到一个名为`bootstrap-5.3.2-dist.zip`的压缩包文件，接下来的工作就是要该文件解压并将其中路径为`css/bootstrap.min.css`的文件复制到之前创建的`bootstrapCase/styles`目录下，而路径为`js/bootstrap.min.js`的文件则复制到`bootstrapCase/scripts`目录下（如果读者在上一步骤中是使用NPM来下载框架的，那么这两个文件现在已经存在于当前项目的`/node_modules/bootstrap/dist/css`和`/node_modules/bootstrap/dist/js`这两个目录中，直接使用即可）。在这里，之所以要使用框架中文件名带有`min`的文件，是因为这些文件已经经过压缩处理，可以大大提高网页加载速度。

4. 接下来，读者需要使用VS Code编辑器中打开之前创建的`BootstrapCase`目录，并在该目录下的`index.htm`文件中的输入如下代码：
    ...
    **需要注意的是**：上述代码中除了用`<link>`标记引入了Bootstrap框架的CSS样式文件之外，还用`<script>`标记引入了Bootstrap框架的JavaScript脚本文件。由于JavaScript脚本操作对象是HTML文档中的页面元素，所以读者在使用`<script>`标记引入JavaScript脚本文件时，需要将该标记的`defer`属性激活，以确保该JavaScript脚本文件在页面加载完毕之后再执行（关于JavaScript脚本在网页设计工作中的运用，本书将会在第4和第5章中做进一步地详细介绍）。

5. 在保存上述代码之后，读者就可以使用网页浏览器打开`index.htm`文件查看当前网页设计的结果，其外观样式在Google Chrome浏览器中的效果如下图所示。

    ![图2-5](img/2-5.png)

    **图2-5**：基于Bootstrap框架的网页布局示例

在上述演示中，我们首先在项目中引入了Bootstrap框架的CSS样式文件和JavaScript文件（以便能该框架提供的外观样式及其相关的功能），然后使用了该框架提供的样式类来完成网页的整体布局，并安排不同布局元素中的内容。关于页面内容的安排，我们会在后面的章节中做专门介绍，现在先来重点关注网页的整体布局。我们在这里主要采用了组件布局和Flexbox布局两大类布局样式。其中，组件布局类的样式主要运用于导航栏区域，而在作为网页主要区域的章节区域中采用的则是Flexbox布局，具体说明如下：
...
正如读者在上述演示中所见，设计师们在使用Bootstrap框架来进行网页整体设计的时候，通常会优先使用`*-primary`、`*-secondary`、`*-success`、`*-danger`、`*-warning`、`*-info`、`*-light`和`*-dark`这八组预定义的样式类来制定网页基础的配色方案，这八组样式类所代表的语义及其默认颜色具体如下：
...
当然了，上述内容所介绍的是由Bootstrap框架提供的默认配色方案，但实际项目实践中，设计师们通常需要根据委托方的需求采用更有针对性的配色方案（例如根据企业的品牌Logo来设计网页的配色）。在这种情况下，读者通常有以下两种选择：

- 从Bootstrap框架的源文件着手，直接修改上述这八组样式类使用的默认颜色，这就需要利用到一项叫做SCSS的样式表技术来修改Bootstrap框架的默认设置。具体操作如下：
...
- 如果觉得直接修改Bootstrap框架的方式过于复杂，对于一些设计并不复杂的网页，读者也可以选择在加载Bootstrap框架提供的样式文件之后，额外再加载一个自定义的CSS样式文件，然后在该样式文件中利用CSS的层叠特性来对网页中特定元素的颜色进行修改；

#### 使用Bootstrap组件

在完成了网页的整体设计之后，设计师们接下来要做的就是安排网页中要展示的各种具体元素了。这部分工作的主要任务是对图文类网页进行图文信息的排版，或者对交互类网页进行用户界面的设计。面对这些任务。Bootstrap框架提供了一系列具有专用功能的页面组件，以便设计师们能以更简单便捷的方式设计出符合要求的网页。虽然图文信息的排版和用户界面的设计都属于本书后续章节中要讨论的主题，但基于本章项目的任务需要，本书在这里会先带读者简单体验一下图文轮播和图文卡片这两个组件的使用方法，这也可以为后续章节中的项目实践奠定一个基础。

- **图文轮播组件**：如果读者想让图片元素以幻灯片的形式在页面中自动播放，就可以考虑使用由Bootstrap框架提供的、一个名为“轮播”的专用组件来进行辅助设计，下面是该组件的一个简单使用示例：
...
- **图文卡片组件**：如果读者想在页面中设置一些图文混合的元素，可以考虑使用Bootstrap框架中提供的卡片组件来进行辅助设计，下面是该组件的一个简单使用示例：
...
在完成了上述知识准备之后，读者现在就可以根据之前【任务书】中的要求来着手实施“凌雪冰熊”网站的首页设计了，该项目的实施过程可以分为以下步骤来进行。

### 第1步：创建凌雪冰熊的官方网站项目

在这一步骤中，网页设计师的主要任务是为凌雪冰熊的委托方创建一个官方网站项目，并在其中引入接下来要使用的Bootstrap框架，以便为后续章节要实施的各个网页设计子项目奠定一个基础。为此，读者需要执行以下操作。

1. 先使用Powershell或Bash Shell这类命令行终端环境打开`Examples`目录，并通过执行`mkdir 02_SnowBear`命令来创建本章项目的根目录。  

2. 继续在命令行终端环境中使用`cd 02_SnowBear`进入到本章项目的根目录下，并分别通过执行`mkdir img`和`mkdir styles`这两个命令为本章项目创建两个子目录，这两个子目录分别用于存放图片素材和自定义的样式表文件。

3. 接着在项目的根目录下使用命令行终端环境执行`npm install bootstrap@5.3.2 --dev`命令，以便将接下来要使用的Bootstrap框架引入到当前项目中，框架的存放路径为项目根目录下的`node_modules/bootstrap`目录。

4. 然后使用VS Code编辑器打开这个刚刚创建的`02_SnowBear`项目，并执行以下文件操作：
   1. 在项目的根目录下创建一个名为`index.htm`的空文件。
   2. 在项目的`styles`子目录下创建一个名为`main.css`的空文件。
   3. 将项目委托方提供的图片资源文件复制到项目的`img`子目录下。
...
5. 最后回到之前的命令行终端环境中，并在项目的根目录中通过执行以下命令来完成本章项目的第一次版本控制操作。

    ```bash
    git init
    git add .
    git commit -m "项目2：创建凌雪冰熊的官方网站"
    ```

### 第2步：完成网站首页的布局设计

在这一步骤中，网页设计师的主要任务是使用HTML 5提供的布局类标记完成网站首页的布局设计，为此需要执行以下操作。

1. 在VS Code编辑器中打开`02_SnowBear`项目，并找到该项目根目录下的`index.htm`文件，由于在默认情况下，网页服务器会自动将文件名为`index`或`default`的HTML文档作为网站的首页发送给网页浏览器，所以读者对于网站首页的设计工作就将在这个文件中进行。

2. 正如本书在上一章中所介绍的，网页设计工作通常得从定义网页的文档结构，和引入要使用的CSS样式文件与JavaScript脚本文件这两件事开始。所以接下来，读者需要在VS Code编辑器中打开`index.htm`文件，并先在其中输入以下代码。
    ...
    需要注意的是：上述代码中除了引入了Bootstrap框架的CSS样式文件与JavaScript脚本文件之外，还引入了自定义的CSS样式文件。该自定义的样式文件主要用于设置一些与凌雪冰熊Logo相关的配色方案，其具体代码如下。
...
3. 在定义好网页的文档结构之后，接下来就可以利用HTML 5提供的布局类标记来进行网站首页的布局设计了。本书在这里会建议读者先从导航栏区域的设计开始，具体做法是回到`index.htm`文件中，找到`<!-- 从这里开始网页的布局设计 -->`这一行注释所在的位置，并将其替换为以下代码。
...
4. 接下来继续页面主体区域的布局设计，本书在这里会建议读者使用`<main>`标记定义这一区域，具体做法是回到`index.htm`文件中，找到`<!-- 导航栏区域结束 -->`这一行注释所在的位置，并该位置后面继续添加以下代码。
    ...
    对于上述代码中提到的、用于显示网站首页中具体内容元素的Bootstrap组件，笔者会将它们留在后面的第3步骤中来设置，读者在这里暂时只需将注意力集中在网页的布局设计即可。

5. 再接下来要进行的是页脚区域的布局设计，本书在这里会建议读者使用`<footer>`标记定义这一区域，具体做法是回到`index.htm`文件中，找到`<!-- 主体区域结束 -->`这一行注释所在的位置，并该位置后面继续添加以下代码。
...
6. 在保存上述代码之后，使用网页浏览器打开`index.htm`文件查看当前网页设计的结果，其外观在Google Chrome浏览器中的效果如图2-6所示。

    ![图2-6](./img/2-6.png)

    **图2-6**：网站首页的整体布局

7. 最后回到之前的命令行终端环境中，并在项目的根目录中通过执行以下命令来完成本章项目的第二次版本控制操作。

    ```bash
    git add .
    git commit -m "项目2：完成网页的整体布局设计"

### 第3步：填充网站首页的内容元素

在这一步骤中，网页设计师的主要任务是为网站的首页填充要展示的内容元素。在这部分的任务中，本书会先带读者来体验一下Bootstrap框架的组件给网页设计工作所带来的便利，以便为后续章节中对具体组件的学习打下一个基础。下面，请跟随笔者执行以下操作来完成本章项目的最后一部分工作。

1. 首先，我们需要使用一个被称之为“轮播”的Bootstrap组件来展示凌雪冰熊饮料店在北京、上海、广州三地的代表性门店，为此，读者需要回到`index.htm`文件中，找到`<!-- 此处将设置一个组件轮播各地门店的照片 -->`这一行注释所在的位置，并将其替换为以下代码。
    ...
    <div id="carouselExample" class="carousel slide" 
        ...
        <div class="carousel-inner">
            <div class="carousel-item active">
                <img src="./img/shop1.jpg"  height="650vh"
            ...
            <div class="carousel-item">
                <img src="./img/shop2.jpg" height="650vh"
                    class="d-block w-100" alt="...">
                <div class="carousel-caption text-start">
                    <h2>上海门店</h2>
                    <ul class="list-unstyled">
                        <li>联系人：林经理</li>       
                        <li>地址：上海市浦东新区XXX</li>
                        <li>电话：234-567-8901</li>
                    </ul>          
                </div>
            </div>
            <div class="carousel-item">
                <img src="./img/shop3.jpg" height="650vh"
                    class="d-block w-100" alt="...">
                <div class="carousel-caption text-start">
                    <h2>广州门店</h2>
                    <ul class="list-unstyled">
...
2. 接下来，我们需要使用三个被称之为“卡片”的Bootstrap组件来展示凌雪冰熊英灵殿的供应体系，为此，读者需要回到`index.htm`文件中，找到`<!-- 此处将设置若干个组件介绍连锁店的供应链 -->`这一行注释所在的位置，并将其替换为以下代码。

    ```html
    <h1 class="text-center text-light my-5 fw-bolder">
        一起感受来自冰雪世界的熊抱！
    </h1>
    <article class="row mt-5 gx-1">
        <div class="card col-4 text-center p-3">
            <i class="fas fa-user-friends fa-5x card-img-top"></i>
            <div class="card-body">
                <h4 class="card-title">研发生产</h4>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">拥有独立的研发团队</li>
                    <li class="list-group-item">严格把控优质原料</li>
                    <li class="list-group-item">拥有多项专利技术</li>
                </ul>
            </div>
        </div>
        <div class="card col-4 text-center text-bg-secondary p-3">
            <i class="fas fa-truck fa-5x card-img-top"></i>
            <div class="card-body">
                <h4 class="card-title">物流运作</h4>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">拥有遍布全国的物流中心</li>
                    <li class="list-group-item">由点到面推行物流免运费</li>
                    <li class="list-group-item">构建强大而高效的物流服务</li>
                </ul>
            </div>
        </div>
        <div class="card col-4 text-center p-3">
            <i class="fas fa-tasks fa-5x card-img-top"></i>
            <div class="card-body">
                <h4 class="card-title">运营管理</h4>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">拥有统一高效的品牌管理</li>
                    <li class="list-group-item">严控各地门店的服务质量</li>
                    <li class="list-group-item">努力提升品牌的知名度与集中力</li>
                </ul>
            </div>
        </div>
    </article>
...
3. 在保存上述代码之后，使用网页浏览器打开`index.htm`文件查看当前网页设计的结果，其外观在Google Chrome浏览器中的效果如图2-7所示。
...
[^1] 自iPhone 12发布之后。各大主流手机制造商在屏幕设计上的选择都逐渐趋于统一，因此这里对于那之后的产品就不再进行罗列了。

因此，设计师们在设计网页时首先做的就是让网页在被载入时自动获取当前网页浏览器的视口尺寸。为了完成好这项工作，本书在这里会推荐读者使用Google Chrome浏览器的调试工具来模拟各种移动端设备，具体方法是通过单击调试工具界面顶部工具栏中的移动设备图标（如图2-4所示）来打开该调试工具的移动端模式：

![图2-8](./img/2-8.png)

**图2-8**：Google Chrome调试工具的移动端模式
...
### 知识点2：基于Bootstrap框架的响应式设计

如果读者想更快速地构建基于响应式设计的网站，更好的选择就是使用Bootstrap框架提供的、专用于响应式设计的样式类了。在命名规则上，Bootstrap框架对这些专用于响应式设计的样式类都是添加了`-sm`、`-md`、`-lg`和`-xl`后缀的，这些后缀的具体含义如下所示：
...
例如，对于本章项目使用到的栅格布局系统和导航栏组件，Bootstrap框架为它提供了一下扩展类：

- **网格布局系统**：在Bootstrap框架中，我们可以通过以下扩展类对栅格布局方式进行补充，使其能更好地适应不同的屏幕尺寸：
...
客户林宇一刚刚创建了一家名为“白熊前端”的培训机构，他希望为该机构设计一个网站首页，并在首页的导航栏中为将来要设计的课程简介、报名流程、原创教程、学员社区四个页面预留位置。你是他的学长，并且现在已经是一位经验丰富的网页设计师，他找到了你，并对网站首页的设计提出了以下需求。

- 项目名：白熊前端官方网站的首页设计
- 委托方：林宇一
- **项目资料**：
  - **图片资料**：白熊前端品牌的Logo（如图2-9所示）。  
    ![图2-9](./img/2-9.jpg)  
    **图2-9**：白熊前端品牌的Logo
- **项目要求**：为白熊前端的官方网站设计首页，该网页的设计应符合以下要求。
  - 该网页的配色需与品牌的Logo保持一致；
  - 该网页应立足于整个网站来设计统一的布局风格；
  - 该网页应配备导航栏功能，并为后续网页的设计项目预留位置。
- 时间要求：在3个工作日内完成；
...
| ----- | --------- | ------------ | --- | ------ | ------ | ---- | --------- |
| 01 | 网页信息呈现 | 网页中是否呈现了甲方提供的基本信息？| 20 |   |   |   |   |  
| 02 | 网页样式呈现 | 网页外观样式是否展示了企业的品牌形象？| 20 |    |   |   |   |
| 03 |  PC端浏览器呈现 | 网页呈现效果在Chrome和Firefox这两款主流网页浏览器中是否一致？| 30   |   |   |   |
| 04 |  移动端浏览器呈现 | 网页呈现效果在iOS和Android的各型主流设备中是否一致？| 30   |   |   |   |
