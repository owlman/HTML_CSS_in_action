...
在本章，笔者将会以一家名为“凌雪冰熊”的连锁饮料店的需求为例来演示如何完成企业网站的首页设计，以便向公众展示该饮料店的品牌信息，以便建立起人们对这家连锁品牌的第一印象。同时，该项目也会为该企业网站后续要设计的新闻活动、产品展示、加盟咨询、留言板等页面设置好统一的布局风格与配色方案，并预留整合这些页面的导航链接。通过本章项目的实践，读者将会初步了解设计一个企业展示类页面所要执行的基本步骤，以及执行这些步骤所需的基本技术与相关工具。总而言之，在阅读完本章之后，我们希望读者能够：
...
现在你是一位刚刚入职到一家名为“凌雪冰熊”连锁饮料店的网页设计师。该连锁店的领导层决定为企业开发一个官方网站，以便向公众更好地展示自己的品牌形象，企业最新的活动，并为潜在的合作伙伴提供咨询信息，进一步扩展线下实体店的加盟规模。在这个网页设计项目中，你的主要任务有两个：首先是为该企业网站完成首页部分的设计，以便为该连锁饮料店建立良好的品牌印象。其次，你需要为该网站设计统一的布局风格与外观样式，为后续要进行的新闻活动、加盟咨询、留言板等网页的设计项目打好基础。
...
4. 接下来，读者需要使用VS Code编辑器中打开之前创建的`BootstrapCase`目录，并在该目录下的`index.htm`文件中的输入如下代码：
    ...
    **需要注意的是**：上述代码中除了用`<link>`标记引入了Bootstrap框架的CSS样式文件之外，还用`<script>`标记引入了Bootstrap框架的JavaScript脚本文件。由于JavaScript脚本操作对象是HTML文档中的页面元素，所以读者在使用`<script>`标记引入JavaScript脚本文件时，需要将该标记的`defer`属性激活，以确保该JavaScript脚本文件在页面加载完毕之后再执行。
...
在这一步骤中，网页设计师的主要任务是为凌雪冰熊的委托方创建一个官方网站项目，并在其中引入接下来要使用的Bootstrap框架，以便为后续章节要实施的各个网页设计子项目奠定一个基础。
...
4. 最后回到之前的命令行终端环境中，并在项目的根目录中通过执行以下命令来完成本章项目的第二次版本控制操作。
